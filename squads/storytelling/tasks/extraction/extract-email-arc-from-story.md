...
- **Rework cycle:** 30-60 minutes (50% of original)
- **Max rework cycles:** 3

## Conditional Paths
- **IF quality gate passes on first submission:** Proceed to registry update and Copy Squad handoff.
...
- **Fields to populate:**
  - `id`: Auto-generated (email-arc-YYYYMMDD-NNN)
  - `project`: Project name from brief
  - `version`: 1.0.0 (new) or increment (revision)
  - `status`: draft → reviewed → approved
  - `quality_score`: NQS from quality gate
  - `created_by`: dan-harmon (primary)
  - `frameworks_used`: [harmon-story-circle, email-sequence-narrative-framework]
  - `linked_assets`: Source story ID, email-sequence-narrative template
